...
[NOTE]
====
Operators previously installed through Operator Lifecycle Manager (OLM) follow a different process for updates. See link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/operators/#olm-upgrading-operators[Updating installed Operators] for more information.
====
...
* xref:update-service-overview_updating-restricted-network-cluster-osus[Using the OpenShift Update Service in a disconnected environment]
* xref:understanding-update-channels_understanding-update-channels-releases[Update channels]

[id="{context}-additional-resources"]
[role="_additional-resources"]
== Additional resources
* xref:how-updates-work[How cluster updates work].
...
[role="_additional-resources"]
.Additional resources
* xref:update-conditional-upgrade-pathupdating-cluster-cli[Updating along a conditional upgrade path]
* xref:fast-stable-channel-strategies_understanding-update-channels-releases[Choosing the correct channel for your cluster]
...
[role="_additional-resources"]
.Additional resources
* xref:update-evaluate-availability_how-updates-work[Evaluation of update availability]
...
[role="_additional-resources"]
.Additional resources
* xref:understanding_clusteroperator_conditiontypes_understanding-openshift-updates[Understanding cluster Operator condition types]
...
* Have a recent link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/backup_and_restore/#backup-etcd[etcd backup] in case your update fails and you must restore your cluster to a previous state.
* Have a recent link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/backup_and_restore/#[Container Storage Interface (CSI) volume snapshot] in case you need to restore persistent volumes due to a pod failure.
...
// * Review the list of APIs that were removed in Kubernetes 1.28, migrate any affected components to use the new API version, and provide the administrator acknowledgment. For more information, see xref:updating-cluster-prepare[Preparing to update to {product-title} 4.14].
...
[role="_additional-resources"]
.Additional resources
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/architecture/#unmanaged-operators_architecture-installation[Support policy for unmanaged Operators]
...
* For information on which machine configuration changes require a reboot, see the note in link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/architecture/#about-machine-config-operator_control-plane[About the Machine Config Operator].
...
* xref:eus-eus-update[Performing an EUS-to-EUS update]
* xref:update-conditional-upgrade-pathupdating-cluster-cli[Updating along a conditional update path]
* xref:understanding-update-channels-releases[Understanding update channels and releases]
...
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/operators/#olm-upgrading-operators[Updating installed Operators]
* xref:update-upgrading-web_updating-cluster-web-console[Updating a cluster by using the web console]
...
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/operators/#olm-upgrading-operators[Updating installed Operators]
* xref:updating-eus-to-eus-upgrade-console_eus-to-eus-update[EUS-to-EUS update using the web console]
* xref:updating-eus-to-eus-upgrade-cli_eus-to-eus-update[EUS-to-EUS update using the CLI]
...
* xref:update-upgrading-web_updating-cluster-web-console[Updating a cluster using the web console]
* xref:update-upgrading-cli_updating-cluster-cli[Updating a cluster using the CLI]
...
* xref:updating-restricted-network-cluster-OSUS[Updating a cluster in a disconnected environment using the OpenShift Update Service]

* xref:updating-restricted-network-cluster[Updating a cluster in a disconnected environment without the OpenShift Update Service]
...
* xref:uninstalling-osus[Uninstalling the OpenShift Update Service from a cluster]
...
. If you use the xref:mirroring-ocp-resources-ocmirror[oc-mirror OpenShift CLI (`oc`) plugin]:
...
. If you use the xref:update-mirror-repository-adm-release-mirror_mirroring-ocp-image-repository[`oc adm release mirror` command]:
...
* For information about viewing the CRI-O logs to view the image source, see link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/installing/#viewing-the-image-pull-source_validating-an-installation[Viewing the image pull source].
...
* xref:oc-mirror-imageset-config-params_mirroring-ocp-image-repository[Image set configuration parameters]
* xref:oc-mirror-image-set-examples_mirroring-ocp-image-repository[Image set configuration examples]
* xref:update-service-about_understanding-openshift-updates[About the OpenShift Update Service]
...
To mirror an image set in a fully disconnected environment, you must first xref:oc-mirror-mirror-to-disk_mirroring-ocp-image-repository[mirror the image set to disk], then xref:oc-mirror-disk-to-mirror_mirroring-ocp-image-repository[mirror the image set file on disk to a mirror].
...
* xref:oc-mirror-updating-cluster-manifests_mirroring-ocp-image-repository[Configuring your cluster to use the resources generated by oc-mirror]
...
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/operators/#olm-managing-custom-catalogs-fb[File-based catalogs]
...
If the release images are contained in a registry whose HTTPS X.509 certificate is signed by a custom certificate authority, complete the steps in link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/registry/#images-configuration-cas_configuring-registry-operator[Configuring additional trust stores for image registry access] along with following changes for the update service.
...
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/operators/#olm-installing-operators-in-namespace[Installing Operators in your namespace].
...
[NOTE]
====
See link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/networking/#enable-cluster-wide-proxy[Configuring the cluster-wide proxy] to configure the CA to trust the update server.
====
...
See link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/operators/#olm-restricted-networks[Using Operator Lifecycle Manager on restricted networks] for more information about how to update your OLM catalog in a disconnected environment.
====
...
** xref:updating-cluster-web-console[Updating a cluster using the web console]
** xref:updating-cluster-cli[Updating a cluster using the CLI]
** xref:eus-eus-update[Performing an EUS-to-EUS update]
** xref:update-using-custom-machine-config-pools[Performing a canary rollout update]
** xref:updating-cluster-rhel-compute[Updating a cluster that includes RHEL compute machines]
...
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/nodes/#nodes-nodes-working-evacuating_nodes-nodes-working[Understanding how to evacuate pods on nodes]
...
[role="_additional-resources"]
.Additional resources
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/postinstallation_configuration/#multi-architecture-configuration[Configuring multi-architecture compute machines on an {product-title} cluster]
* xref:updating-cluster-web-console[Updating a cluster using the web console]
* xref:updating-cluster-cli[Updating a cluster using the CLI]
* xref:understanding-clusterversion-conditiontypes_understanding-openshift-updates[Understanding cluster version condition types]
* xref:understanding-update-channels-releases[Understanding update channels and releases]
...
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/machine_management/#machine-health-checks-about_deploying-machine-health-checks[About machine health checks]
...
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/support/#support_gathering_data_gathering-cluster-data[Gathering data about your cluster]
