...
* Configuring an identity provider: You can define any xref:supported-identity-providers[supported identity provider in {product-title}] and add it to your cluster.

* xref:configuring-internal-oauth[Configuring the internal OAuth server]: The {product-title} control plane includes a built-in OAuth server that determines the user's identity from the configured identity provider and creates an access token. You can configure the token duration and inactivity timeout, and customize the internal OAuth server URL.
+
[NOTE]
====
Users can xref:managing-oauth-access-tokens[view and manage OAuth tokens owned by them].
====

* Registering an OAuth client: {product-title} includes several xref:oauth-default-clients_configuring-oauth-clients[default OAuth clients]. You can xref:oauth-register-additional-client_configuring-oauth-clients[register and configure additional OAuth clients].
+
[NOTE]
====
...
* Managing cloud provider credentials using the xref:about-cloud-credential-operator[Cloud Credentials Operator]: Cluster components use cloud provider credentials to get permissions required to perform cluster-related tasks.
* Impersonating a system admin user: You can grant cluster administrator permissions to a user by xref:impersonating-system-admin[impersonating a system admin user].
...
* Creating a cluster role and assigning it to a user or group: {product-title} includes a set of xref:default-roles_using-rbac[default cluster roles]. You can create additional xref:creating-cluster-role_using-rbac[cluster roles] and xref:adding-roles_using-rbac[add them to a user or group].

* Creating a cluster-admin user: By default, your cluster has only one cluster administrator called `kubeadmin`. You can xref:creating-cluster-admin_using-rbac[create another cluster administrator]. Before creating a cluster administrator, ensure that you have configured an identity provider.
+
[NOTE]
====
After creating the cluster admin user, xref:removing-kubeadmin_removing-kubeadmin[delete the existing kubeadmin user] to improve cluster security.
====
...
|xref:configuring-htpasswd-identity-provider[htpasswd]
...
|xref:configuring-keystone-identity-provider[Keystone]
...
|xref:configuring-ldap-identity-provider[LDAP]
...
|xref:configuring-basic-authentication-identity-provider[Basic authentication]
...
|xref:configuring-request-header-identity-provider[Request header]
...
|xref:configuring-github-identity-provider[GitHub or GitHub Enterprise]
...
|xref:configuring-oidc-identity-provider[OpenID Connect]
...
xref:authorization-overview_using-rbac[use RBAC to define and apply permissions].
...
* For information about creating a service account token secret, see link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/nodes/#nodes-pods-secrets-creating-sa_nodes-pods-secrets[Creating a service account token secret].
...
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/security_and_compliance/#nodes-nodes-audit-log-basic-viewing_audit-log-view[Viewing audit logs]
...
xref:configuring-ldap-identity-provider[Configuring an LDAP identity provider].
...
* xref:configuring-ldap-identity-provider[Configuring an LDAP identity provider]
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/nodes/#nodes-nodes-jobs-creating-cron_nodes-nodes-jobs[Creating cron jobs]
...
* **xref:cco-mode-mint[Mint]**: In mint mode, the CCO uses the provided admin-level cloud credential to create new credentials for components in the cluster with only the specific permissions that are required.

* **xref:cco-mode-passthrough[Passthrough]**: In passthrough mode, the CCO passes the provided cloud credential to the components that request cloud credentials.

* **xref:cco-mode-manual[Manual mode with long-term credentials for components]**: In manual mode, you can manage long-term cloud credentials instead of the CCO.

* **xref:cco-short-term-creds[Manual mode with short-term credentials for components]**: For some providers, you can use the CCO utility (`ccoctl`) during installation to implement short-term credentials for individual components. These credentials are created and managed outside the {product-title} cluster.
...
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/operators/#cloud-credential-operator_cluster-operators-ref[Cluster Operators reference page for the Cloud Credential Operator]
...
[NOTE]
====
By default, mint mode requires storing the `admin` credential in the cluster `kube-system` namespace. If this approach does not meet the security requirements of your organization, you can link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/postinstallation_configuration/#manually-removing-cloud-creds_post-install-cluster-tasks[remove the credential after installing the cluster].
====
...
[role="_additional-resources"]
== Additional resources
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/postinstallation_configuration/#manually-removing-cloud-creds_post-install-cluster-tasks[Removing cloud provider credentials]
...
[NOTE]
====
xref:cco-mode-manual[Manual mode] is the only supported CCO configuration for Microsoft Azure Stack Hub.
====
...
To locate the `CredentialsRequest` CRs that are required, see link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/installing/#manually-create-iam_installing-aws-customizations[Manually creating long-term credentials for AWS].
...
To locate the `CredentialsRequest` CRs that are required, see link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/installing/#manually-create-iam_installing-azure-customizations[Manually creating long-term credentials for Azure].
...
To locate the `CredentialsRequest` CRs that are required, see link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/installing/#manually-create-iam_installing-gcp-customizations[Manually creating long-term credentials for GCP].
...
If `CredentialsRequest` CRs change over time as the cluster is upgraded, you must manually update the passthrough mode credential to meet the requirements. To avoid credentials issues during an upgrade, check the `CredentialsRequest` CRs in the release image for the new version of {product-title} before upgrading. To locate the `CredentialsRequest` CRs that are required for your cloud provider, see _Manually creating long-term credentials_ for link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/installing/#manually-create-iam_installing-aws-customizations[AWS], link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/installing/#manually-create-iam_installing-azure-customizations[Azure], or link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/installing/#manually-create-iam_installing-gcp-customizations[GCP].
...
To locate the `CredentialsRequest` CRs that are required for AWS, Azure, or GCP and learn how to change the permissions the CCO uses, see _Manually creating long-term credentials_ for link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/installing/#manually-create-iam_installing-aws-customizations[AWS], link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/installing/#manually-create-iam_installing-azure-customizations[Azure], or link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/installing/#manually-create-iam_installing-gcp-customizations[GCP].
