...
As a cluster administrator, you might need to stop an {product-title} cluster for a period and restart it later. Some reasons for restarting a cluster are that you need to perform maintenance on a cluster or want to reduce resource costs. In {product-title}, you can perform a xref:graceful-shutdown-cluster[graceful shutdown of a cluster] so that you can easily restart the cluster later.

You must xref:backup-etcd[back up etcd data] before shutting down a cluster; etcd is the key-value store for {product-title}, which persists the state of all resource objects. An etcd backup plays a crucial role in disaster recovery. In {product-title}, you can also xref:replacing-unhealthy-etcd-member[replace an unhealthy etcd member].

When you want to get your cluster running again, xref:graceful-restart-cluster[restart the cluster gracefully].

[NOTE]
====
A cluster's certificates expire one year after the installation date. You can shut down a cluster and expect it to restart gracefully while the certificates are still valid. Although the cluster automatically retrieves the expired control plane certificates, you must still xref:dr-recovering-expired-certs[approve the certificate signing requests (CSRs)].
====
...
You can always recover from a disaster situation by xref:dr-restoring-cluster-state[restoring your cluster to its previous state] using the saved etcd snapshots.
...
OADP backs up and restores Kubernetes resources and internal images, at the granularity of a namespace, by using the version of Velero that is appropriate for the version of OADP you install, according to the table in xref:velero-obtaining-by-downloading_oadp-troubleshooting[Downloading the Velero CLI tool].  OADP backs up and restores persistent volumes (PVs) by using snapshots or Restic. For details, see xref:oadp-features_oadp-features-plugins[OADP features].
...
You back up applications by creating a `Backup` custom resource (CR). See xref:backing-up-applications[Creating a Backup CR]. You can configure the following backup options:

* xref:backing-up-applications[Creating backup hooks] to run commands before or after the backup operation

* xref:backing-up-applications[Scheduling backups]

* xref:backing-up-applications[Backing up applications with File System Backup: Kopia or Restic]

* You restore application backups by creating a `Restore` (CR). See xref:oadp-creating-restore-cr_restoring-applications[Creating a Restore CR].
...
* Take an xref:backing-up-etcd-data_backup-etcd[etcd backup] prior to shutting down the cluster.
+
[IMPORTANT]
====
...
* xref:graceful-restart-cluster[Restarting the cluster gracefully]
...
You configure AWS for Velero, create a default `Secret`, and then install the Data Protection Application. For more details, see link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/null/#oadp-installing-operator-doc[Installing the OADP Operator].
...
You configure Azure for Velero, create a default `Secret`, and then install the Data Protection Application. For more details, see link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/null/#oadp-installing-operator-doc[Installing the OADP Operator].
...
You configure GCP for Velero, create a default `Secret`, and then install the Data Protection Application. For more details, see link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/null/#oadp-installing-operator-doc[Installing the OADP Operator].
...
You create a `Secret` for the backup location and then you install the Data Protection Application. For more details, see link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/null/#oadp-installing-operator-doc[Installing the OADP Operator].
...
You can configure xref:installing-oadp-mcg[Multicloud Object Gateway] or any AWS S3-compatible object storage as a backup location.
...
You back up persistent volumes with Container Storage Interface (CSI) snapshots by editing the `VolumeSnapshotClass` custom resource (CR) of the cloud storage before you create the `Backup` CR, see link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/storage/#persistent-storage-csi-snapshots-overview_persistent-storage-csi-snapshots[CSI volume snapshots].

For more information, see xref:oadp-creating-backup-cr-doc[Creating a Backup CR].
...
* You can use OADP Data Mover to back up Container Storage Interface (CSI) volume snapshots to a remote object store. See xref:oadp-using-data-mover-for-csi-snapshots-doc[Using Data Mover for CSI snapshots].

* You can use OADP 1.2 Data Mover to backup and restore application data for clusters that use CephFS, CephRBD, or both. See xref:oadp-using-data-mover-for-csi-snapshots-doc[Using OADP 1.2 Data Mover with Ceph storage].

* You must perform a data cleanup after you perform a backup, if you are using OADP 1.1 Data Mover. See xref:oadp-cleaning-up-after-data-mover-1-1-backup-doc[Cleaning up after a backup using OADP 1.1 Data Mover].
...
OADP 1.3 includes a built-in Data Mover that you can use to move Container Storage Interface (CSI) volume snapshots to a remote object store. The built-in Data Mover allows you to restore stateful applications from the remote object store if a failure, accidental deletion, or corruption of the cluster occurs. It uses xref:oadp-about-kopia[Kopia] as the uploader mechanism to read the snapshot data and write to the unified repository.
...
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/architecture/#[Admission plugins]
...
|Set specific resource `limits` and `requests` for a `Velero` pod or a `Restic` pod as described in xref:oadp-setting-resource-limits-and-requests_installing-oadp-aws[Setting Velero CPU and memory resource allocations].
...
xref:dr-restoring-cluster-state[Restoring to a previous cluster state]::
...
+
If applicable, you might also need to xref:dr-recovering-expired-certs[recover from expired control plane certificates].
+
[WARNING]
====
...
Prior to performing a restore, see xref:dr-scenario-2-restoring-cluster-state-about_dr-restoring-cluster-state[About restoring cluster state] for more information on the impact to the cluster.
====
+
[NOTE]
====
If you have a majority of your masters still available and have an etcd quorum, then follow the procedure to xref:replacing-unhealthy-etcd-member[replace a single unhealthy etcd member].
====

xref:dr-recovering-expired-certs[Recovering from expired control plane certificates]::
