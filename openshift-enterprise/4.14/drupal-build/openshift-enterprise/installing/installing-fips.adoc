...
To ensure that the secrets that are stored in etcd use FIPS validated or Modules In Process encryption, boot the node in FIPS mode. After you install the cluster in FIPS mode, you can link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/security_and_compliance/#encrypting-etcd[encrypt the etcd data] by using the FIPS-approved `aes cbc` cryptographic algorithm.
...
in xref:installing-customizing[Customizing nodes].
...
* xref:installing-aws-customizations[Amazon Web Services]
* xref:installing-alibaba-customizations[Alibaba Cloud]
* xref:installing-azure-customizations[Microsoft Azure]
* xref:installing-bare-metal[Bare metal]
* xref:installing-gcp-customizations[Google Cloud Platform]
* xref:installing-ibm-cloud-customizations[{ibm-cloud-name}]
* xref:installing-ibm-power[{ibm-power-name}]
* xref:installing-ibm-z[{ibm-z-name} and {ibm-linuxone-name}]
* xref:installing-ibm-z-kvm[{ibm-z-name} and {ibm-linuxone-name} with {op-system-base} KVM]
* xref:installing-openstack-installer-custom[{rh-openstack-first}]
* xref:installing-vsphere[VMware vSphere]
...
To apply `AES CBC` encryption to your etcd data store, follow the link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/security_and_compliance/#encrypting-etcd[Encrypting etcd data] process after you install your cluster.

If you add {op-system-base} nodes to your cluster, ensure that you enable FIPS mode on the machines before their initial boot. See link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html-single/machine_management/#adding-rhel-compute[Adding RHEL compute machines to an {product-title} cluster] and link:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html/security_hardening/using-the-system-wide-cryptographic-policies_security-hardening#enabling-fips-mode-in-a-container_using-the-system-wide-cryptographic-policies[Enabling FIPS Mode] in the {op-system-base} 8 documentation.
