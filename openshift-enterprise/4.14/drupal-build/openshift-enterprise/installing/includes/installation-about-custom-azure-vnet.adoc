...
include::./custom-dns-server.adoc[]
...
[.small]
--
1. If you are using Azure Firewall to restrict the internet access, then xref:configuring-firewall[you can configure Azure Firewall to allow the Azure APIs]. A network security group rule is not needed.
--

include::./mcs-endpoint-limitation.adoc[]
...
* xref:configuring-firewall[Configuring your firewall]
