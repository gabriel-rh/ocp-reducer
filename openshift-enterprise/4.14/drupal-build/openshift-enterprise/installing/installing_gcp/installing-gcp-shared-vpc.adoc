...
* You xref:installing-gcp-account[configured a GCP project] to host the cluster. This project, known as the service project, must be attached to the host project. For more information, see link:https://cloud.google.com/vpc/docs/provisioning-shared-vpc#create-shared[Attaching service projects in the GCP documentation].
* You have a GCP service account that has the xref:minimum-required-permissions-ipi-gcp-xpn[required GCP permissions] in both the host and service projects.
...
include::../includes/installation-gcp-shared-vpc-config.adoc[leveloffset=+2]
...
* To manage long-term cloud credentials manually, follow the procedure in xref:manually-create-iam_installing-gcp-shared-vpc[Manually creating long-term credentials].

* To implement short-term credentials that are managed outside the cluster for individual components, follow the procedures in xref:installing-gcp-with-short-term-creds_installing-gcp-shared-vpc[Configuring a GCP cluster to use short-term credentials].
