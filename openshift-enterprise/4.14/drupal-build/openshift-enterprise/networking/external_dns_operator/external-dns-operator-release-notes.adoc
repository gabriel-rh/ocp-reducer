...
* The External DNS Operator now supports AWS shared VPC. For more information, see xref:nw-control-dns-records-public-aws-with-VPC_creating-dns-records-on-aws[Creating DNS records in a different AWS Account using a shared VPC].
