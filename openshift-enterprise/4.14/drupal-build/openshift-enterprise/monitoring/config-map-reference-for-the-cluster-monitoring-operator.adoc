...
These configurations are defined by xref:clustermonitoringconfiguration[ClusterMonitoringConfiguration].
...
These configurations are defined by xref:userworkloadconfiguration[UserWorkloadConfiguration].
...
Appears in: xref:prometheusk8sconfig[PrometheusK8sConfig],
xref:prometheusrestrictedconfig[PrometheusRestrictedConfig],
xref:thanosrulerconfig[ThanosRulerConfig]
...
|tlsConfig|xref:tlsconfig[TLSConfig]|Defines the TLS settings to use for Alertmanager connections.
...
Appears in: xref:clustermonitoringconfiguration[ClusterMonitoringConfiguration]
...
Appears in: xref:userworkloadconfiguration[UserWorkloadConfiguration]
...
|===
| Property | Type | Description
|alertmanagerMain|*xref:alertmanagermainconfig[AlertmanagerMainConfig]|`AlertmanagerMainConfig` defines settings for the Alertmanager component in the `openshift-monitoring` namespace.

|enableUserWorkload|*bool|`UserWorkloadEnabled` is a Boolean flag that enables monitoring for user-defined projects.

|k8sPrometheusAdapter|*xref:k8sprometheusadapter[K8sPrometheusAdapter]|`K8sPrometheusAdapter` defines settings for the Prometheus Adapter component.

|kubeStateMetrics|*xref:kubestatemetricsconfig[KubeStateMetricsConfig]|`KubeStateMetricsConfig` defines settings for the `kube-state-metrics` agent.

|metricsServer|*xref:metricsserverconfig[MetricsServerConfig]|`MetricsServer` defines settings for the Metrics Server component.

|prometheusK8s|*xref:prometheusk8sconfig[PrometheusK8sConfig]|`PrometheusK8sConfig` defines settings for the Prometheus component.

|prometheusOperator|*xref:prometheusoperatorconfig[PrometheusOperatorConfig]|`PrometheusOperatorConfig` defines settings for the Prometheus Operator component.

|prometheusOperatorAdmissionWebhook|*xref:prometheusoperatoradmissionwebhookconfig[PrometheusOperatorAdmissionWebhookConfig]|`PrometheusOperatorAdmissionWebhookConfig` defines settings for the admission webhook component of Prometheus Operator.

|openshiftStateMetrics|*xref:openshiftstatemetricsconfig[OpenShiftStateMetricsConfig]|`OpenShiftMetricsConfig` defines settings for the `openshift-state-metrics` agent.

|telemeterClient|*xref:telemeterclientconfig[TelemeterClientConfig]|`TelemeterClientConfig` defines settings for the Telemeter Client component.

|thanosQuerier|*xref:thanosquerierconfig[ThanosQuerierConfig]|`ThanosQuerierConfig` defines settings for the Thanos Querier component.

|nodeExporter|xref:nodeexporterconfig[NodeExporterConfig]|`NodeExporterConfig` defines settings for the `node-exporter` agent.

|monitoringPlugin|*xref:monitoringpluginconfig[MonitoringPluginConfig]|`MonitoringPluginConfig` defines settings for the monitoring `console-plugin` component.
...
Appears in: xref:k8sprometheusadapter[K8sPrometheusAdapter]
...
|dedicatedServiceMonitors|*xref:dedicatedservicemonitors[DedicatedServiceMonitors]|Defines dedicated service monitors.
...
Appears in: xref:nodeexportercollectorconfig[NodeExporterCollectorConfig]
...
Appears in: xref:nodeexporterconfig[NodeExporterConfig]

[options="header"]
|===
| Property | Type | Description
|cpufreq|xref:nodeexportercollectorcpufreqconfig[NodeExporterCollectorCpufreqConfig]|Defines the configuration of the `cpufreq` collector, which collects CPU frequency statistics. Disabled by default.

|tcpstat|xref:nodeexportercollectortcpstatconfig[NodeExporterCollectorTcpStatConfig]|Defines the configuration of the `tcpstat` collector, which collects TCP connection statistics. Disabled by default.

|netdev|xref:nodeexportercollectornetdevconfig[NodeExporterCollectorNetDevConfig]|Defines the configuration of the `netdev` collector, which collects network devices statistics. Enabled by default.

|netclass|xref:nodeexportercollectornetclassconfig[NodeExporterCollectorNetClassConfig]|Defines the configuration of the `netclass` collector, which collects information about network devices. Enabled by default.

|buddyinfo|xref:nodeexportercollectorbuddyinfoconfig[NodeExporterCollectorBuddyInfoConfig]|Defines the configuration of the `buddyinfo` collector, which collects statistics about memory fragmentation from the `node_buddyinfo_blocks` metric. This metric collects data from `/proc/buddyinfo`. Disabled by default.

|mountstats|xref:nodeexportercollectormountstatsconfig[NodeExporterCollectorMountStatsConfig]|Defines the configuration of the `mountstats` collector, which collects statistics about NFS volume I/O activities. Disabled by default.

|ksmd|xref:nodeexportercollectorksmdconfig[NodeExporterCollectorKSMDConfig]|Defines the configuration of the `ksmd` collector, which collects statistics from the kernel same-page merger daemon. Disabled by default.

|processes|xref:nodeexportercollectorprocessesconfig[NodeExporterCollectorProcessesConfig]|Defines the configuration of the `processes` collector, which collects statistics from processes and threads running in the system. Disabled by default.

|systemd|xref:nodeexportercollectorsystemdconfig[NodeExporterCollectorSystemdConfig]|Defines the configuration of the `systemd` collector, which collects statistics on the systemd daemon and its managed services. Disabled by default.
...
|===
| Property | Type | Description
|collectors|xref:nodeexportercollectorconfig[NodeExporterCollectorConfig]|Defines which collectors are enabled and their additional configuration parameters.
...
|===
| Property | Type | Description
|additionalAlertmanagerConfigs|[]xref:additionalalertmanagerconfig[AdditionalAlertmanagerConfig]|Configures additional Alertmanager instances that receive alerts from the Prometheus component. By default, no additional Alertmanager instances are configured.
...
|remoteWrite|[]xref:remotewritespec[RemoteWriteSpec]|Defines the remote write configuration, including URL, authentication, and relabeling settings.
...
Appears in: xref:clustermonitoringconfiguration[ClusterMonitoringConfiguration],
xref:userworkloadconfiguration[UserWorkloadConfiguration]
...
Appears in: xref:additionalalertmanagerconfig[AdditionalAlertmanagerConfig]
...
|===
| Property | Type | Description
|additionalAlertmanagerConfigs|[]xref:additionalalertmanagerconfig[AdditionalAlertmanagerConfig]|Configures how the Thanos Ruler component communicates with additional Alertmanager instances. The default value is `nil`.
...
|===
| Property | Type | Description
|alertmanager|*xref:alertmanageruserworkloadconfig[AlertmanagerUserWorkloadConfig]|Defines the settings for the Alertmanager component in user workload monitoring.

|prometheus|*xref:prometheusrestrictedconfig[PrometheusRestrictedConfig]|Defines the settings for the Prometheus component in user workload monitoring.

|prometheusOperator|*xref:prometheusoperatorconfig[PrometheusOperatorConfig]|Defines the settings for the Prometheus Operator component in user workload monitoring.

|thanosRuler|*xref:thanosrulerconfig[ThanosRulerConfig]|Defines the settings for the Thanos Ruler component in user workload monitoring.
